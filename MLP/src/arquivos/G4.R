...
rm(list=ls(all=TRUE))
data<-read.table("G4 - Base_Simulacao.txt")
...
heartValues <- heart[,1:24]
heartTargets <- heart[, 28]
#25-Infeccao_Global  
#26-Obito	
#27-Tempo_Internacao	
#28-Infeccao_Sitio_Cirurgico

heartDecTargets <- decodeClassLabels(heartTargets)
heart <- splitForTrainingAndTest(heartValues, heartDecTargets, ratio = 0.20)
...
model <- mlp(heart$inputsTrain, heart$targetsTrain, size = 2,learnFunc="BackpropWeightDecay",
